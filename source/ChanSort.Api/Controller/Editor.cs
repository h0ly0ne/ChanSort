...
namespace ChanSort.Api
{
    public class Editor
    {
        ...
        {
            int count = channels.Count(channel => channel.GetPosition(SubListIndex) == -1);

            if (count == 0) 
                return null;

            ChannelInfo lastInsertedChannel = null;
            int progNr = ChannelList.InsertProgramNumber;
            int relativeChannelNumber = 0;
            int progNrCopy = progNr; // prevent "access to modified closure" warning

            foreach (var channel in ChannelList.Channels.Where(c => c.GetPosition(SubListIndex) >= progNrCopy).OrderBy(c => c.GetPosition(SubListIndex)))
            {
                var curPos = channel.GetPosition(SubListIndex);
                int gap = count - (curPos - progNr - relativeChannelNumber);

                if (gap > 0)
                {
                    channel.SetPosition(SubListIndex, curPos + gap);
            ...
            {
                if (channel.GetPosition(SubListIndex) != -1)
                    continue;

                channel.SetPosition(SubListIndex, progNr++);
                lastInsertedChannel = channel;
            }

            ChannelList.InsertProgramNumber += count;
            DataRoot.NeedsSaving |= lastInsertedChannel != null;
        ...
        public void RemoveChannels(IList<ChannelInfo> channels, bool closeGap)
        {
            ...
            ChannelList.InsertProgramNumber = channels[0].GetPosition(SubListIndex);
            IOrderedEnumerable<ChannelInfo> orderedChannelList = ChannelList.Channels.Where(c => c.GetPosition(SubListIndex) != -1).OrderBy(c => c.GetPosition(SubListIndex));
            ...
            foreach (var channel in channels)
            {
                ...
                if (closeGap)
                {
                    int prevNr = channel.GetPosition(SubListIndex);

                    foreach (var channel2 in orderedChannelList)
                    {
                        if (channel2.GetPosition(SubListIndex) > channel.GetPosition(SubListIndex))
                        {
                            // ignore deleted and proxy channels (prevNr<0), broken channels (==0) and channels after a gap
                            if (prevNr <= 0 || channel2.GetPosition(SubListIndex) != prevNr + 1)
                                break;

                            prevNr = channel2.GetPosition(SubListIndex);
                            channel2.ChangePosition(SubListIndex, -1);
                        }
                    }
                }
                channel.SetPosition(SubListIndex, -1);
            }
        ...
        public void MoveChannels(IList<ChannelInfo> channels, bool up)
        {
            ...
            if (up && channels[0].GetPosition(SubListIndex) <= ChannelList.FirstProgramNumber)
                return;
            ...
            {
                int newProgramNr = channel.GetPosition(SubListIndex) + delta;
                ChannelInfo channelAtNewPos = ChannelList.Channels.FirstOrDefault(ch => ch.GetPosition(SubListIndex) == newProgramNr);
                channelAtNewPos?.ChangePosition(SubListIndex, -delta);
                channel.ChangePosition(SubListIndex, delta);
            }
            DataRoot.NeedsSaving = true;
        }
        #endregion
        ...
        public void SortSelectedChannels(List<ChannelInfo> selectedChannels)
        {
            ...
            sortedChannels.Sort(ChannelComparerForSortingByName);
            var programNumbers = selectedChannels.Select(ch => ch.GetPosition(SubListIndex)).ToList();

            for (int i = 0; i < sortedChannels.Count; i++)
            {
                sortedChannels[i].SetPosition(SubListIndex, programNumbers[i]);
            }
        ...
        {
            return string.Compare(channel1.Name, channel2.Name, StringComparison.Ordinal);
        }
        #endregion
        ...
        public void SetSlotNumber(IList<ChannelInfo> channels, int slot, bool swap, bool closeGap)
        {
            ...
            if (swap)
            {
                foreach (var channel in channels)
                {
                    if (slot != -1)
                    {
                        var others = ChannelList.GetChannelByNewProgNr(slot);

                        foreach (var other in others)
                        {
                            other.SetPosition(SubListIndex, channel.GetPosition(SubListIndex));
                        }
                    }

                    channel.SetPosition(SubListIndex, slot++);
                }
            }
            else
            {
                RemoveChannels(channels, closeGap);
                ChannelList.InsertProgramNumber = slot;
        ...
        public void RenumberChannels(List<ChannelInfo> channels)
        {
            ...
            int progNr = channels.Min(ch => ch.GetPosition(SubListIndex));

            foreach (var channel in channels)
            {
                if (channel.GetPosition(SubListIndex) == progNr)
                {
                    ++progNr;
                    continue;
                }

                var list = new List<ChannelInfo> { channel };
                RemoveChannels(list, false);
                ChannelList.InsertProgramNumber = progNr++;
                AddChannels(list);
                DataRoot.NeedsSaving = true;
            }
        }
        ...
        public void ApplyReferenceList(DataRoot refDataRoot, ChannelList refList, ChannelList tvList, bool addProxyChannels = true, int positionOffset = 0, Func<ChannelInfo, bool, bool> chanFilter = null, bool overwrite = true, bool consecutive = false)
        {
            ...
            foreach (var refChannel in refList.Channels.OrderBy(ch => ch.OldProgramNr))
            {
                ...
                if (tvChannel != null)
                {
                    ...
                    if (refDataRoot.CanSkip && DataRoot.CanSkip)
                        tvChannel.Skip = refChannel.Skip;

                    if (refDataRoot.CanLock && DataRoot.CanLock)
                        tvChannel.Lock = refChannel.Lock;

                    if (refDataRoot.CanHide && DataRoot.CanHide)
        ...
        {
            return ((long)onid << 32) | ((long)tsid << 16) | sid;
        }

        private void ApplyFavorites(DataRoot refDataRoot, ChannelInfo refChannel, ChannelInfo tvChannel)
        {
            if (DataRoot.SortedFavorites)
            {
                if (!DataRoot.MixedSourceFavorites || refDataRoot.MixedSourceFavorites)
                {
        ...
        {
            bool sortedFav = DataRoot.SortedFavorites;
            var favMask = (Favorites)(1 << favIndex);
            var favList = DataRoot.ChannelLists.FirstOrDefault(l => l.IsMixedSourceFavoritesList) ?? ChannelList;
            ...
            else
            {
                ...
                if (sortedFav && !DataRoot.AllowGapsInFavNumbers)
                {
                    int i = 0;
        ...
        {
            var supMask = (int)DataRoot.SupportedFavorites;
