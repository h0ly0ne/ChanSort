...
namespace ChanSort.Api
{
    public static class Tools
    {
        ...
        public static V TryGet<K, V>(this IDictionary<K, V> dict, K key, V defaultValue = default)
        {
        ...
        {
            if (freq < 41) return string.Empty;
            if (freq <= 68) return ((freq - 41)/7 + 1).ToString("d2"); // Band I (01-04)
            if (freq < 105) return string.Empty;
        ...
        {
            return littleEndian ? BitConverter.ToInt32(data, offset) : (data[offset] << 24) + (data[offset + 1] << 16) + (data[offset + 2] << 8) + data[offset + 3];
        }
        #endregion
        ...
        public static byte[] HexDecode(string input)
        {
            ...
            for (int i = 0, c = input.Length/2; i < c; i++)
            {
                ...
                int high = Char.IsDigit(ch) ? ch - '0' : ch - 'A' + 10;
                ch = Char.ToUpper(input[i*2 + 1]);
                int low = Char.IsDigit(ch) ? ch - '0' : ch - 'A' + 10;
        ...
        public static string HexEncode(byte[] bytes, bool uppercase = false)
        {
            ...
            string hexDigits = uppercase ? HexDigitsUpper : HexDigitsLower;
