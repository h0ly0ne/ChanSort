    ...
    */
    ...
    public class DvbStringDecoder
    {
        ...
        public void GetChannelNames(byte[] name, int off, int len, out string longName, out string shortName)
        {
            ...
            Decoder decoder = DefaultEncoding.GetDecoder();
            ...
            if (b < 0x20)
            {
                ...
                {
                    SetDecoder(codePages2, name[off + 1] * 256 + name[off + 2], ref decoder);
                    off += 2;
                    len -= 2;
                }
            ...
            if (!singleByteChar)
            {
                ...
                longName = new string(buffer, 0, decoder.GetChars(name, off, len, buffer, 0, false));
        ...
        private void SetDecoder(string[] codePages, int cpIndex, ref Decoder defaultDecoder)
        {
            ...
            if (decoderCache.TryGetValue(cp, out defaultDecoder))
                return;

            try
            {
                defaultDecoder = Encoding.GetEncoding(cp).GetDecoder();
            }
            catch { /* ignored */ }
        ...
        public static byte[] GetCodepageBytes(Encoding encoding)
        {
            ...
            {
                if (codePages1[i] == encoding.WebName)
                    return new [] {(byte)i};
            }

            for (int i = 0; i < codePages2.Length; i++)
            {
                if (codePages2[i] == encoding.WebName)
